...
void lprotocol_datagram_print(lprotocoldatagram_t * pdatagram)
{
	...
	for(i=0;i<pdatagram->datagram_size && i<15;++i){
		printf("%2Xh ",(unsigned char)pdatagram->datagram[i]);
	}
	if((i==15) && (i<=pdatagram->datagram_size)) printf("...");
	printf("\n datagram_size: %i",pdatagram->datagram_size);
	
	lprotocol_datagram_decoder_retrieve_data(&protocoldata, pdatagram);
	printf("\n function_code: %2Xh",protocoldata.function_code);
	...
	for(n=0;n<protocoldata.number_of_data;++n){
		...
		for(i=0;i<protocoldata.data_size[n] && i<15;++i){
			printf("%2Xh ",(unsigned char)(*(protocoldata.pdata[n]+i)));
		}
	}
}
#endif
	...
	unsigned int datagramindex = 0;
		...
		if (pdatagram->datagram_size > 4){
			...
			while(datagramindex<(pdatagram->datagram_size-1)){
				pprotocoldata->data_size[pprotocoldata->number_of_data] = (unsigned char)pdatagram->datagram[3];
...
lprotocolreturnstatus_t lprotocol_datagram_decoder_process_received_byte(unsigned char received_byte, lprotocoldatagram_t *pdatagram)
{
	...
	case LPROTOCOL_DATAGRAM_STATE_EMPTY: // 0
			...
			if (received_byte == LPROTOCOL_DATAGRAM_HEADER){
				...
				pdatagram->state = LPROTOCOL_DATAGRAM_STATE_HEADER_RECEIVED;
			}
		#endif
		break;
		
	case LPROTOCOL_DATAGRAM_STATE_HEADER_RECEIVED: // 1
		#if LPROTOCOL_DATAGRAM_ENCODING	== LPROTOCOL_DATAGRAM_ENCODING_SHORT_EQUALDATAFIELDSSIZE
			pdatagram->datagram[pdatagram->datagram_size++] = received_byte;
			pdatagram->checksum += received_byte;
			pdatagram->state = LPROTOCOL_DATAGRAM_STATE_FUNCTION_RECEIVED;
		#endif
		break;
		
	case LPROTOCOL_DATAGRAM_STATE_FUNCTION_RECEIVED:  // 2
		#if LPROTOCOL_DATAGRAM_ENCODING	== LPROTOCOL_DATAGRAM_ENCODING_SHORT_EQUALDATAFIELDSSIZE
			pdatagram->datagram[pdatagram->datagram_size++] = received_byte;
			pdatagram->checksum += received_byte;
			pdatagram->state = LPROTOCOL_DATAGRAM_STATE_DATAGRAMSIZE_RECEIVED;
			if(pdatagram->datagram_size>=(pdatagram->datagram[2]-1)){ // ver se não tem campo de dados por vir. falta só o checksum
				pdatagram->state = LPROTOCOL_DATAGRAM_STATE_DATAFIELD_RECEIVED;
			} 
		#endif
		break;
		
	case LPROTOCOL_DATAGRAM_STATE_DATAGRAMSIZE_RECEIVED:  // 3
		#if LPROTOCOL_DATAGRAM_ENCODING	== LPROTOCOL_DATAGRAM_ENCODING_SHORT_EQUALDATAFIELDSSIZE
			pdatagram->datagram[pdatagram->datagram_size++] = received_byte;
			pdatagram->checksum += received_byte;
	...
	case LPROTOCOL_DATAGRAM_STATE_DATAFIELD_RECEIVED:  // 4
		#if LPROTOCOL_DATAGRAM_ENCODING	== LPROTOCOL_DATAGRAM_ENCODING_SHORT_EQUALDATAFIELDSSIZE
			pdatagram->datagram[pdatagram->datagram_size++] = received_byte;
